/*
    xm_play.cpp - main sound playing function
		 ...
		 void *userData )
{
    ...
    //temp variables: ============
    int temp;
    int i2;           //current buffer pointer
    int i;            //current buffer pointer (for small block)
    int rest;         //rest size
    int size;
    int cc;           //current channel number
    channel *ch;       //current channel pointer
    sample *smp;       //current sample pointer
    int s_offset;     //sample data offset
    int left;         //left channel
    int right;        //right channel
    int s;            //current piece of sample (volume from -32767 to 32767)
    int sr;           //the same as above but for the next channel (right)
    signed short *pnt; //sample pointer (16 bit data)
    signed char *cpnt; //sample pointer (8 bit data)
    int reppnt;       //sample repeat pointer
    int replen;       //sample repeat length
    int res;	       //result volume
    pattern *pat;      //current pattern
    xmnote *nptr;      //pattern pointer
    xmnote *n;         //current note pointer
    int cur_tablepos;
    //interpolation variables:
    int in;           //interpolation scale
    int in2;          //interpolation scale
    int s2;           //next piece of sample
    int s_offset2;    //sample data offset (for interpolate)
    //frequency interpolation:
    int old_subtick_c;//start subtick count for current tick's piece
    int new_freq;     //new sample frequency
    //============================
...
next_iteration:
		...
		if( U->sample_int ) 
		{
		    ...
		    if( s_offset < (int)smp->length - 1 ) s_offset2 = s_offset + 1; else s_offset2 = s_offset;
		    //loop correction: =====
		    if( replen )
		    {
		...
		if( ch->period_delta ) 
		{
		    ...
		    ch->delta_p = ch->delta & ( ( (int)1 << XM_PREC ) - 1 );
		    ch->delta >>= XM_PREC;
		}
	    }
	    //****************************
...
if(note<97) { \
	...
	cptr->delta_p = cptr->delta & ( ( (int)1 << XM_PREC ) - 1 ); \
	cptr->delta >>= XM_PREC; \
	RETRIG_SAMPLE \
    } \
    else { \
...
void worknote( xmnote *nptr, int virt_cnum, int cnum, xm_struct *U )
{
    ...
    uint note, inst, vol, period, freq, fx, par;
    uint sample_num = 0;
    uint events;
    int i;
    instrument *ins;
	...
	case 0xB: if( !U->jump_flag ) 
		  {
			...
			{ 
			    if( U->status == XM_STATUS_PLAYLIST && par <= (uint)U->tablepos )
				U->song_finished = 1;
...
void envelope( int cnum, xm_struct *U ) //***> every tick <***
{
    channel *cptr = U->channels[ cnum ];
    if( !cptr->smp ) return;
    ...
    int pan_value = 128;
    int pan_mul;
    int l_vol, r_vol; //current envelope volume (0..1024)

    int pos;      //vibrato position
